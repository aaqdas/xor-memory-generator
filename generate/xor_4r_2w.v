...
module xor_4r_2w #(parameter ADDR_WIDTH = 10,parameter DATA_WIDTH  = 32)
                (input clk
                  ,input [1:0] i_enW
...
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage1(.data(dIn[0]),
                .write_addr(wa1_regd),.read_addr(i_ra4),.we(enW_regd[0]),.clk(clk),.q(dOut1));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage2(.data(dIn[0]),
                .write_addr(wa1_regd),.read_addr(i_ra3),.we(enW_regd[0]),.clk(clk),.q(dOut2));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage3(.data(dIn[0]),
                .write_addr(wa1_regd),.read_addr(i_ra2),.we(enW_regd[0]),.clk(clk),.q(dOut3));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage4(.data(dIn[0]),
                .write_addr(wa1_regd),.read_addr(i_ra1),.we(enW_regd[0]),.clk(clk),.q(dOut4));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage5(.data(dIn[1]),
                .write_addr(wa2_regd),.read_addr(i_wa1),.we(enW_regd[1]),.clk(clk),.q(dOut5));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage6(.data(dIn[1]),
                .write_addr(wa2_regd),.read_addr(i_ra4),.we(enW_regd[1]),.clk(clk),.q(dOut6));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage7(.data(dIn[1]),
                .write_addr(wa2_regd),.read_addr(i_ra3),.we(enW_regd[1]),.clk(clk),.q(dOut7));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage8(.data(dIn[1]),
                .write_addr(wa2_regd),.read_addr(i_ra2),.we(enW_regd[1]),.clk(clk),.q(dOut8));
simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage9(.data(dIn[1]),
                .write_addr(wa2_regd),.read_addr(i_ra1),.we(enW_regd[1]),.clk(clk),.q(dOut9));
always @ (*)
begin
dIn[0] = w1_regd ^ dOut5;
dIn[1] = w2_regd ^ dOut0;
