...
with open(f"./generate/{op_file_name}.v",'w') as file:
    ...
    file.write(f"module {op_file_name} #(parameter ADDR_WIDTH = {int(np.ceil(np.log2(size/(word_size/8))))},\
parameter DATA_WIDTH  = {word_size})\n\
                (input clk\n"    
               f"                  ,input [{write_ports-1}:0] i_enW\n")
    for i in range(0,read_ports):
    ...
    for i in range(0,i_count):
        ...
        for j in range(0,j_count):
            file.write(f'''simple_dual_port_ram #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) stage{i*j_count+j}(.data(dIn[{i}]),
                .write_addr(wa{i+1}_regd),.read_addr({"i_wa" if j < write_ports-1 else "i_ra"}{order.flatten()[i*j_count+j]}),.we(enW_regd[{i}]),.clk(clk),.q(dOut{i*j_count+j}));\n''')
